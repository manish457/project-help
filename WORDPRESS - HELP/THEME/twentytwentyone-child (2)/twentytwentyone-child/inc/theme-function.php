			...
			if($parent_page > 0){
				...
				if(get_post_type() != 'page' ){
					if(get_post_type() == 'gallery'):
						$template_file_path = $templates.'template.gallery.php';
					elseif(get_post_type() == 'doctor'):
						$template_file_path = $templates.'template.doctor-overview.php';
					else:
	...
	function simple_breadcrumb() {
		...
		if (!is_front_page()) {
			...
			if ( is_category() || is_single() ) {
				if(get_post_type()==='gallery') {
					$template_file_path = $templates.'template.gallery.php';
				}else if($post_type==='doctor'){
					$template_file_path = $templates.'template.doctor-overview.php';
				}else if($post_type==='videos'){
					$template_file_path = $templates.'template.video-overview.php';
				}else if($post_type==='team'){
					$template_file_path = $templates.'template.meet-the-team.php';
				}else if($post_type==='patient'){
					if(is_single()) $template_file_path = '';
				}else{
					$template_file_path = $templates.'template.blog-overview.php';
				}
				$template_post_id =  get_id_by_template_name($template_file_path);
				if($template_post_id){
...
if (!function_exists('parent_gallery_category_breadcrumb')) {
	function parent_gallery_category_breadcrumb($id, $taxonomy_slug, $output = '', $label = 0){
		...
		$name = $label == 0 ? '<strong>'.$term->name. ' ' . '' . '</strong>' : '<strong>' .$term->name. '</strong>';//(get_post_type()==='gallery' ? __('Gallery', 'twentychild') : '')
...
function pagination($pages = '', $range = 2){
    ...
    if(1 != $pages)
    {
		...
		/*::::::::::::: NEW <1...56789...19>  ::::::::::::*/
		$prev_html = "<img src=\"".get_stylesheet_directory_uri()."/assets/images/prev.png\"/>";
		$next_html = "<img src=\"".get_stylesheet_directory_uri()."/assets/images/next.png\"/>";
		echo "<div class=\"pagination\">";
		echo "<ul>";
		if($paged > 1) {echo "<li class=\"prev\"><a href='". get_pagenum_link($paged - 1) . "'>".$prev_html."</a></li>"; } else {echo "<li class=\"prev no-element\">".$prev_html."</li>"; }
		if(($paged-3)>0) {
		...
		echo $output;
		if ($paged < $pages) {echo "<li class=\"next\"><a href=\"".get_pagenum_link($paged + 1) . "\">".$next_html."</a></li>"; } else {echo "<li class=\"next no-element\">".$next_html."</li>"; }
		echo "</ul>";
        echo "</div>\n";
		echo '<div class="clearfix"></div>';
    }
    ...
    if (!is_admin() && is_search()) :
        ...
        $orderby =
            ...
            CASE WHEN {$wpdb->prefix}posts.post_type = 'page' THEN '1' 
                 ...
                 WHEN {$wpdb->prefix}posts.post_type = 'patient-stories' THEN '4' 
				 WHEN {$wpdb->prefix}posts.post_type = 'in-the-media' THEN '5' 
				 WHEN {$wpdb->prefix}posts.post_type = 'team' THEN '6' 
